...
- 正式網站沿用原 Sites 專案與網址，已部署 Sites v83；對應來源提交為 `a5139a99c577cbe0f68c365a775560dc58ff82c6`。
...
- Windows 等價 Vinext 正式建置成功，完整 Node 測試 266/266 通過，部署封裝通過 Worker entrypoint 與 hosting manifest 驗證。
...
- 根目錄 `VERSION_HISTORY.md` 保留 v1～v82 歷史索引並追加 v83 修改、防退步與部署收據；往後每次建立／部署版本前必須先追加，不得覆寫舊紀錄。

## 本次完成

- v83：門片有無、片數、開向、開口或完成寬高、斜把與加工集中在最後一關整批確認；每輪完整重讀，最多3輪。前輪草稿不再鎖死後輪尺寸，完整結果立即採用，不再交給其他AI重判。
- 門與加工保持同輪索引；加工清單只含4E，鋁框只作排除註記。原桶身門草稿不能覆蓋新結果。缺項以逐桶逐組的寬／高／加工位置等明確訊息回報，並留下不含原圖與秘密的診斷。
...
- 先前第 5／6 張全圖漏掃、非標準板厚資料流、字型 404 與 19 桶裁切保留修正也都包含在目前 v83。

## 可重現驗證

在 `cabinet-ai-demo`：

- 完整離線測試：`node --test tests/*.test.mjs` → 266/266；新增10項最後門面整批确认回歸。本輪未執行付費AI原圖掃描。
